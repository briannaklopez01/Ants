...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        current = self.place
        bee_find = current.bees
        range_bee = 0
        while current != beehive:
            if bee_find != []:
                if range_bee <= self.max_range and range_bee >= self.min_range:
                    return random_or_none(bee_find)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        lst_of_bees = self.place.bees[:]
        self.armor = self.armor - amount
        if self.armor <= 0:
            for bee in lst_of_bees:
        ...
        if self.id == 0:
            ...
            while current != None:
                if current.ant is not None :
                    if current.ant.is_updated == False:
                        current.ant.damage *= 2
                        current.ant.is_updated = True
                    if isinstance(current.ant,ContainerAnt) and current.ant.contained_ant and current.ant.contained_ant.is_updated==False:
                        current.ant.contained_ant.damage *= 2
                        current.ant.contained_ant.is_updated = True
...
def make_scare(action, bee):
    ...
    def scared(gamestate):
        bee.forward_direction = True
        action(gamestate)
        bee.forward_attribute = False
    return scared
    # END Problem EC
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    original = bee.action
    new_effect = effect(bee.action,bee)
    def applyer(gamestate):
        nonlocal duration
        if duration == 0:
            bee.forward_direction = False
            return original(gamestate)
        else:
            duration -= 1
            return new_effect(gamestate)
    bee.action = applyer
